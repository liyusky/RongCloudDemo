var RongCallLib = function(b) {
  var a = (function() {
    function c(f) {
      ...
      g._memorySessions.client.on("stream-added", function(h) {
        var i = h.stream;
        g._memorySessions.client.subscribe(i, function(j) {
          console.log("Subscribe stream failed", j)
        })
      });
      g._memorySessions.client.on("peer-leave", function(h) {
      ...
      g._memorySessions.client.on("stream-subscribed", function(h) {
        var i = h.stream;
        g._memorySessions.remoteStreamList[i.getId()] = i;
        g.displayStream(i)
      });
      g._memorySessions.client.on("stream-removed", function(h) {
        var i = h.stream
      })
    }
    c.init = function(d) {
    ...
    c.prototype.hungupCall = function(l, i, f) {
      ...
      if (k) {
        var d = new b.HungupMessage({callId: k.Id, reason: b.ErrorCode.REMOTE_HANGUP});
        g.sendMessage(l, i, d)
      }
      var h = new b.HungupMessage({callId: b.Bridge._client.userId, reason: f});
      var e = new b.Message();
    ...
    c.prototype.joinCall = function(d, g) {
      ...
      c._rongIMClient.getAgoraDynamicKey(1, f.callId, {
        onSuccess: function(h) {
          e._memorySessions.client.init(h.dynamicKey, function(i) {
            var j = new b.AcceptMessage({callId: f.callId, mediaType: f.mediaType});
            e.sendMessage(e._memorySessions.message.conversationType, e._memorySessions.message.targetId, j, {
              onSuccess: function(k) {
                e._memorySessions.client.join(h.dynamicKey, f.callId, k.sentTime & 2147483647, function(l) {
    ...
    c.prototype.changeMediaType = function(f, e, d, j) {
      var h = this,
        ...
        i = new b.MediaModifyMessage({callId: g.Id, mediaType: b.VoIPMediaType.MEDIA_AUDIO});
      if (d == b.VoIPMediaType.MEDIA_VEDIO) {
        ...
        if (d == b.VoIPMediaType.MEDIA_AUDIO) {
          if (!h._memorySessions.localStream.audioEnabled) {
            ...
            h._memorySessions.client.disableAudio(h._memorySessions.localStream, function() {
              j.onSuccess()
            })
          } else {
            h._memorySessions.localStream.audioEnabled = false;
            h._memorySessions.client.enableAudio(h._memorySessions.localStream, function() {
              j.onSuccess()
            })
          }
        }
    ...
    c.prototype.mute = function(f) {
      ...
      e.client.disableAudio(d, function() {
        f()
      })
    };
    c.prototype.unmute = function(f) {
      var e = this._memorySessions,
        d = e.localStream;
      d.audioEnabled = true;
      e.client.enableAudio(d, function() {
        f()
      })
    };
    ...
    c.prototype.switchMedia = function(h) {
      var g = this._memorySessions["chnl_" + h.targetId],
        j = new b.MediaModifyMessage({callId: g.Id, mediaType: h.type});
      var f = this;
    ...
    c.prototype.initLocalStream = function(g, d) {
      ...
      if (f._memorySessions.localStream) {
        f._memorySessions.client.unpublish(f._memorySessions.localStream, function(h) {
          console.log("Unpublish failed with error: ", h)
        });
        f._memorySessions.localStream.close()
      }
      f._memorySessions.localStream = AgoraRTC.createStream({streamID: g, local: true});
      if (d == b.VoIPMediaType.MEDIA_AUDIO) {
        ...
        f._memorySessions.localStream.init(function() {
          ...
          f.displayStream();
          f._memorySessions.client.publish(f._memorySessions.localStream, function(i) {});
      ...
      switch (l.messageType) {
        case b.RongIMClient.MessageType.InviteMessage:
          ...
          if (f._memorySessions.isActiveCall) {
            var d = new b.HungupMessage({callId: i.callId, reason: b.ErrorCode.REMOTE_BUSYLINE});
          ...
          var e = new b.RingingMessage({callId: i.callId});
        ...
        case b.RongIMClient.MessageType.AcceptMessage:
          ...
          f._memorySessions.client.init(k.Key, function(m) {
            f._memorySessions.client.join(k.Key, k.Id, f._memorySessions.sentTime & 2147483647, function(n) {
